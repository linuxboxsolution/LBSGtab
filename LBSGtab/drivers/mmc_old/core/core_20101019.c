...
void mmc_rescan(struct work_struct *work)
{
	...
	mmc_bus_get(host);
    NV_DRIVER_TRACE(("mmc_rescan 3\n"));
	/* if there still is a card present, stop here */
	if (host->bus_ops != NULL) {
		mmc_bus_put(host);
		goto out;
	}
    NV_DRIVER_TRACE(("mmc_rescan 4\n"));
	 ...
	 */
	mmc_bus_put(host);
    NV_DRIVER_TRACE(("mmc_rescan 5\n"));
	if (host->ops->get_cd && host->ops->get_cd(host) == 0)
		goto out;
    NV_DRIVER_TRACE(("mmc_rescan 6\n"));
	mmc_claim_host(host);
